...
> Верифицировано по открытым issues в anthropics/claude-code (6 раундов ревью). Актуально на 24 февраля 2026. Все 24 ранее документированных issues проверены — open, описания совпадают. Добавлено 9 новых high-priority issues, найденных в 6-м ревью.
...
Teammates создают orphan inboxes, отправляя сообщения по `agentType` вместо зарегистрированного `name`. Связан с #25135 (name mismatch), но имеет отдельный root cause — agentType vs name confusion в коде SDK.
...
| --- | --- | --- |
| Research (параллельный запуск) | JavaScript heap OOM (#19100) | Ограничивать до 2-3 параллельных субагентов; `model: haiku` |
