...
> Верифицировано по открытым issues в anthropics/claude-code. Актуально на 24 февраля 2026. Итого: 55 issues (53 open, 2 closed). Closed: #18837 (дубликат #14956), #27044 (контекст для #27778).
...
### [Issue #27099] [SECURITY] `allowed-tools:` молча игнорируется в agent frontmatter — субагент наследует все инструменты родителя
**Статус:** открыт (v2.1.49)

Использование `allowed-tools:` (синтаксис Skills) вместо `tools:` (синтаксис agent frontmatter) молча игнорируется. Субагент наследует ВСЕ инструменты родителя, включая неограниченный Bash. Задокументированы инциденты: субагенты обращались к macOS Keychain, извлекали JWT-токены, совершали 85+ несанкционированных tool calls.

**Влияние на методологию:** **КРИТИЧНО** — естественная ошибка (Skills используют `allowed-tools:`, агенты `tools:`) создаёт полный обход ограничений инструментов.

**Обходной путь:** использовать **только** `tools:` в agent frontmatter. Добавить проверку в CI: grep по `allowed-tools:` в `.claude/agents/*.md`.

---

### [Issue #28041] `claude --worktree` не копирует `.claude/` поддиректории в worktree
**Статус:** открыт (v2.1.51, macOS)

`claude --worktree` копирует только `settings.local.json` в `.claude/` нового worktree. Директории `skills/`, `agents/`, `docs/`, `rules/` и `settings.json` отсутствуют. Skills и кастомные агенты недоступны в worktree-сессиях.

**Влияние на методологию:** **КРИТИЧНО** — `isolation: worktree` делает все кастомные агенты из `.claude/agents/` недоступными.

**Обходной путь:** вручную создать symlinks на `.claude/agents/` и `.claude/skills/` после создания worktree, либо использовать WorktreeCreate hook для автоматического копирования.

---

### [Issue #27343] `$CLAUDE_PROJECT_DIR` указывает на main project dir, а не worktree
**Статус:** открыт (v2.1.49, never worked — не регрессия)

`$CLAUDE_PROJECT_DIR` в worktree-сессиях (`claude -w`) разрешается в основную директорию проекта, а не в директорию worktree. Hook-скрипты, использующие эту переменную, работают с неправильными путями.

**Влияние на методологию:** hook-скрипты ломаются в worktree-сессиях. Нельзя полагаться на `$CLAUDE_PROJECT_DIR` для определения рабочей директории.

**Обходной путь:** использовать абсолютные пути в hook-скриптах; определять worktree-путь через `git rev-parse --show-toplevel` вместо `$CLAUDE_PROJECT_DIR`.

---

### [Issue #28126] Task tool: дублирование MCP-серверов и leak `~/.claude/tasks/`
**Статус:** открыт (v2.1.52)

Каждый Task tool субагент запускает полный дубликат всех MCP-серверов (удваивает количество node.exe процессов). При неожиданном завершении родительской сессии директории `~/.claude/tasks/{uuid}/` не удаляются.

**Влияние на методологию:** деградация производительности при частом использовании Task tool с MCP-серверами. Накопление orphan-директорий на диске.

**Обходной путь:** минимизировать количество `mcpServers` у субагентов. Периодически чистить `~/.claude/tasks/`.

---

### [Issue #26851] VSCode: background-субагенты молча deny файловые операции (regression v2.1.47)
**Статус:** открыт (regression в v2.1.47)

В VSCode extension background-субагенты молча отклоняют file write операции вместо запроса разрешения. Прогресс задач невидим (показывается как мгновенно завершённые).

**Влияние на методологию:** background-субагенты непригодны в VSCode для задач с записью файлов. Связан с #13890.

**Обходной путь:** использовать `bypassPermissions` (небезопасно) или CLI вместо VSCode extension для background-субагентов.
...
| Security (agent frontmatter) | **[SECURITY]** `allowed-tools:` молча игнорируется (#27099) | Использовать **только** `tools:` в agent frontmatter; CI-проверка на `allowed-tools:` |
| Research (isolation: worktree) | `.claude/` поддиректории не копируются (#28041) | Symlinks или WorktreeCreate hook для копирования agents/skills |
| Research (isolation: worktree) | `$CLAUDE_PROJECT_DIR` указывает не на worktree (#27343) | `git rev-parse --show-toplevel` вместо `$CLAUDE_PROJECT_DIR` в hooks |
| Task tool + MCP | Дублирование MCP-серверов, leak tasks/ (#28126) | Минимизировать mcpServers у субагентов; чистить ~/.claude/tasks/ |
| VSCode + background | Background субагенты deny file writes (#26851) | CLI вместо VSCode для background-субагентов; или bypassPermissions |
...
- **Не использовать `allowed-tools:` в agent frontmatter** — молча игнорируется, субагент наследует все инструменты (#27099). Только `tools:`
- **Worktree: `.claude/` поддиректории не копируются** — agents, skills, docs недоступны (#28041). Symlinks или WorktreeCreate hook
- **Не использовать `$CLAUDE_PROJECT_DIR` в hook-скриптах** при worktree — указывает на main dir (#27343). Использовать `git rev-parse --show-toplevel`
