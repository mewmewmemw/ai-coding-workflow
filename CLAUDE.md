...
- `/scaffold` -- автономная генерация `.claude/` конфигурации на основе research-файлов. Саморефрентная: генерирует в том числе себя
- `/converge` -- автономный цикл: аудит → верификация → исправление → повторный аудит, пока 0 проблем. Без подтверждений
